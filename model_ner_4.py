...
class NER:
    ...
    def __init__(self, vocab_size_c=5793, vocab_size_p=25, num_classes=53, embedding_size_c=256, embedding_size_p=256,
        ...
        # 可以求平均或者直接拼接
        con_vec = tf.concat((vec_lstm_c, vec_lstm_p, vec_lstm_p, cnn_block_p), 2)

        print("add_vec", con_vec.shape)

        # crf 输出层
        self.classifer(con_vec)
...
def train():
    ...
    with tf.Session(config=config) as sess:
        ...
        ckpt = tf.train.get_checkpoint_state('./ckpt_4/')
        if ckpt:
        ...
        for i in range(num_train_steps):
            ...
            if step % FLAGS.evaluate_every == 0:
                ...
                if total_loss < min_loss:
                    ...
                    min_loss = total_loss
                    saver.save(sess, './ckpt_4/ner.ckpt', global_step=step)
...
class Model:
    def __init__(self):
        ...
        with open('./ckpt_4/ner.pb', "rb") as f:

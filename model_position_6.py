...
class Position:
    ...
    def __init__(self, vocab_size_c=5793, vocab_size_p=25, num_classes=53, embedding_size_c=256, embedding_size_p=32,
        ...
        # p o 预测
        self.loss_o, self.p_o = self.predict_po()
    ...
    def predict_po(self):
        ...
        with tf.name_scope('predict_po'):
            ...
            # print(fc.shape)
            print(self.logits.shape)  # n,m,3
            cc = tf.concat((self.con_vec, self.logits), 2)
            print(cc.shape)
            ...
            return loss_o, viterbi_sequence_o
...
def train():
    ...
    with tf.Session(config=config) as sess:
        ...
        if not os.path.exists('./ckpt_6/'):
            os.makedirs("./ckpt_6/")

        # 恢复模型 / 重新初始化参数
        # model_file = tf.train.latest_checkpoint('./ckpt/')
        ckpt = tf.train.get_checkpoint_state('./ckpt_6/')
        if ckpt:
            ...
            for i in range(num_train_steps):
                ...
                if step % FLAGS.evaluate_every == 0:
                    ...
                    if total_loss < min_loss:
                        ...
                        min_loss = total_loss
                        saver.save(sess, './ckpt_6/ner.ckpt', global_step=step)
...
class Model:
    def __init__(self):
        ...
        with open('./ckpt_6/ner.pb', "rb") as f:

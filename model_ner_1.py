...
from data_3 import get_input_data, load_dict, sequence_padding
...
def train():
    ...
    with tf.Session(config=config) as sess:
        ...
        ckpt = tf.train.get_checkpoint_state('./ckpt_1/')
        if ckpt:
        ...
        min_loss = 99999
        input_ids_train, output_types_train = get_input_data("./train_data_3/train_ner.tf_record", FLAGS.batch_size)
        input_ids_dev, output_types_dev = get_input_data("./train_data_3/dev_ner.tf_record", FLAGS.batch_size)
        for i in range(num_train_steps):
            ...
            if step % FLAGS.evaluate_every == 0:
                ...
                for j in range(num_dev_steps):
                    ...
                    loss, tp, p_, r_ = dev_step(input_ids_dev_, output_types_dev_)
                    TP += tp
                    P_ += p_
                    R_ += r_
                    total_loss += loss
                ...
                if total_loss < min_loss:
                    ...
                    min_loss = total_loss
                    saver.save(sess, './ckpt_2/ner.ckpt', global_step=step)
...
class Model:
    def __init__(self):
        ...
        with open('./ckpt_1/ner.pb', "rb") as f:
        ...
        self.sess.run(init)
        # 读入数据
        self.char2id, self.ner2id = load_dict(char_dict="train_data_3/char2id.json",
...
if __name__ == "__main__":
    ...
    m = Model()
    spo_list = m.predict("《在温柔中疼痛》是由网络作家宋江创作的一部网络文学作品，发表于2008年，正文语种为简体中文，现已完成")
    print(spo_list)
    # m.submit()
